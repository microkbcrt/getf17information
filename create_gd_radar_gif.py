...
def create_guangdong_radar_gif():
    """
    下载广东天气官网最新的10张雷达图并制作成GIF动图。
    直接使用 Pillow 库生成 GIF 以精确控制帧率。
    """
    print("开始执行广东雷达图GIF生成脚本...")

    # 设置时区为北京时间 (UTC+8)
    beijing_tz = timezone(timedelta(hours=8))
    now = datetime.now(beijing_tz)

    # 计算最新的雷达图发布时间点 (通常每6分钟更新)
    ...
    for i in range(10):
        ...
        # 核心修改点：将URL后缀从 _gz.png 改为 _gd.png
        url = f"http://www.tqyb.com.cn/data/swan/mcr/{date_str}/{time_str}_gd.png"
    ...
    image_objects = []
    # 创建一个临时文件夹
    if not os.path.exists("radar_images_gd"):
        os.makedirs("radar_images_gd")
    ...
    for i, url in enumerate(image_urls):
        try:
            ...
            if response.status_code == 200:
                image_path = os.path.join("radar_images_gd", f"radar_{i:02d}.png")
    ...
    if not downloaded_files:
        ...
        # 清理空文件夹
        if os.path.exists("radar_images_gd"):
            os.rmdir("radar_images_gd")
        return
    ...
    try:
        ...
        if image_objects:
            ...
            image_objects[0].save(
                'gd_radar.gif',  # 修改输出文件名
                save_all=True,
                append_images=image_objects[1:],
                duration=200,    # Pillow 的 duration 单位是毫秒
                loop=0           # 0表示无限循环
            )
            print("成功生成GIF文件: gd_radar.gif")
        else:
